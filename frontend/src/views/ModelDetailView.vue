        ...
        <template v-else>
          <div class="dev-stats-row">
            <div class="dev-stat-item">
              ...
              <span class="dev-stat-val">{{ devStats.total }}</span>
            </div>
            <div class="dev-stat-item">
              <span class="dev-stat-label">Open</span>
              <span class="badge dev-open">{{ devStats.open }}</span>
            </div>
            <div class="dev-stat-item">
              <span class="dev-stat-label">Resolve</span>
              <span class="badge dev-resolve">{{ devStats.resolve }}</span>
            </div>
            <div class="dev-stat-item">
              <span class="dev-stat-label">Pending</span>
              <span class="badge dev-pending">{{ devStats.pending }}</span>
            </div>
            <div class="dev-stat-item">
              <span class="dev-stat-label">Close</span>
              <span class="badge dev-close">{{ devStats.close }}</span>
            </div>
          </div>
...
import { getEffectiveNameMonthly, getModelNotes, createModelNote, updateModelNote, deleteModelNote, getModelDevIssues } from '../api'
...
const notes = ref([])
const devIssues = ref([])
const devStats = ref({ total: 0, open: 0, resolve: 0, close: 0, pending: 0 })
...
async function load() {
  ...
  const [statsRes, notesRes, devRes] = await Promise.allSettled([
  ...
  if (devRes.status === 'fulfilled') {
    devStats.value = devRes.value.data.stats || devStats.value
...
/* Dev issues */
.dev-stats-row { display: flex; gap: 16px; flex-wrap: wrap; margin-bottom: 14px; }
.dev-stat-item { display: flex; flex-direction: column; align-items: center; gap: 4px; }
.dev-stat-label { font-size: 0.78rem; color: #888; font-weight: 500; }
.dev-stat-val { font-size: 1.2rem; font-weight: 700; color: #333; }
